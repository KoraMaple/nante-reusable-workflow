...
output "lxc_id" {
  value = var.resource_type == "lxc" && length(proxmox_lxc.container) > 0 ? values(proxmox_lxc.container)[0].vmid : null
...
output "lxc_hostname" {
  value = var.resource_type == "lxc" && length(local.vm_hostnames) > 0 ? values(local.vm_hostnames)[0] : ""
